...
async function debug() {
    ...
    try {
        ...
        const [projects] = await connection.query("SELECT id, name FROM projects WHERE name LIKE '%VRB018%'");
        console.log(projects);

        if (projects.length > 0) {
            const pid = projects[0].id;
            console.log(`\nproject_materials for project ${pid}:`);
            const [materials] = await connection.query("SELECT id, material_type, material_name, quantity FROM project_materials WHERE project_id = ?", [pid]);
