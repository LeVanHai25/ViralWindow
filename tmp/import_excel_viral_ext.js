...
// Configuration
const EXCEL_FILE = path.join('d:', 'ViralWindow_Phan_Mem_Nhom_Kinh', 'Tài liệu', 'Tồn kho nhôm VIRAL kho viral.xlsx');
const TARGET_WAREHOUSE_ID = 1; // Kho Nhôm VIRAL

async function importData() {
    let connection;
    try {
        console.log('🚀 DỰ ÁN: NHẬP DỮ LIỆU TỒN KHO VIRAL (EXTENDED)');
        ...
        // Skip header row
        const rows = rawData.slice(1).filter(row => row[0]); 
        ...
        for (const row of rows) {
            ...
            const length = parseFloat(row[5]) || 6; // Default to 6 if empty per debug info
            const quantity = parseFloat(row[6]) || 0;
            const minStock = parseInt(row[7]) || 5;
            const maxStock = parseInt(row[8]) || 50;
            const color = row[12]?.toString().trim() || 'Xám sần';
            const brand = 'VIRAL';
            const thickness = 1.4;
            ...
            if (existing.length > 0) {
                ...
                console.log(`Replacing data for code: ${code}`);
                await connection.query(
