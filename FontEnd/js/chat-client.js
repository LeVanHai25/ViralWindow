...
function initChat(){
    ...
    socket.on('disconnect',()=>{const el=document.getElementById('connectionStatus');if(el){el.textContent='🔴 Mất kết nối';el.style.color='#ef4444';}});
    socket.on('connect_error',()=>{const el=document.getElementById('connectionStatus');if(el)el.textContent='⚠️ Lỗi';});
...
function renderConversations(filter=''){
    ...
    if(!filtered.length){list.innerHTML='<div style="text-align:center;padding:40px;color:#94a3b8;"><div style="font-size:32px;margin-bottom:8px;">💬</div><p style="font-size:13px;">'+(filter?'Không tìm thấy':'Chưa có cuộc trò chuyện')+'</p></div>';return;}
    list.innerHTML=filtered.map(c=>{
        ...
        return `<div class="chat-conversation-item ${isActive?'active':''}" onclick="openConversation(${c.id})">
            ...
            <div class="chat-conv-info"><div class="chat-conv-name">${c.type==='group'?'👥 ':''}${escHtml(c.display_name||'')}</div><div class="chat-conv-preview">${escHtml(c.last_message_preview||'')}</div></div>
...
async function openConversation(convId){
    ...
    if(hSub){
        if(conv.type==='private'&&conv.other_user_id){
            hSub.textContent=onlineUserIds.has(conv.other_user_id)?'🟢 Online':'⚫ Offline';
...
function renderMessage(msg){
    ...
    const pin=msg.is_pinned?'<span style="font-size:10px;">📌</span> ':'';
    ...
    const emojiBtn=`<span class="msg-emoji-trigger" onclick="event.stopPropagation();toggleEmojiPicker(${msg.id})" style="display:none;position:absolute;top:-8px;${isMe?'left:-30px':'right:-30px'};cursor:pointer;font-size:16px;background:white;border-radius:50%;width:24px;height:24px;text-align:center;line-height:24px;box-shadow:0 1px 4px rgba(0,0,0,0.15);">😊</span>`;
...
function setReply(msgId, senderName, contentPreview) {
    ...
    bar.innerHTML = `<div style="flex:1;min-width:0;"><div style="font-size:11px;font-weight:600;color:#6366f1;">↩️ Trả lời ${escHtml(senderName)}</div><div style="font-size:12px;color:#64748b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escHtml(contentPreview)}</div></div><span onclick="cancelReply()" style="cursor:pointer;font-size:16px;color:#94a3b8;padding:4px;">✕</span>`;
...
function updatePresenceUI(){
    ...
    if(currentConversation){
        ...
        if(hSub&&currentConversation.type==='private'&&currentConversation.other_user_id){
            ...
            hSub.textContent=online?'🟢 Online':'⚫ Offline';hSub.style.color=online?'#22c55e':'#94a3b8';
        }
    }
}
...
function updateTypingUI(){const el=document.getElementById('typingIndicator');if(!el)return;if(!typingUsers.size){el.textContent='';return;}el.textContent=`✏️ ${Array.from(typingUsers.values()).join(', ')} đang gõ...`;}
...
function showMsgContextMenu(evt,msgId,isPinned){
    ...
    const pinBtn=document.createElement('div');pinBtn.innerHTML=isPinned?'📌 Bỏ ghim':'📌 Ghim tin nhắn';pinBtn.style.cssText='padding:8px 12px;cursor:pointer;font-size:13px;border-radius:6px;';pinBtn.onmouseover=function(){this.style.background='#f1f5f9';};pinBtn.onmouseout=function(){this.style.background='';};pinBtn.onclick=function(){pinMessage(msgId);};
